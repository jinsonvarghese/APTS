...
This appendix covers platforms that split an engagement across multiple agents, workers, or model-driven components. It does not create new conformance requirements. The point is narrower than that. APTS already has controls for scope, safety, logging, containment, and human oversight. Those controls were mostly written against a single agent runtime. Once a platform starts dividing work across several concurrent decision makers, the control logic still applies, but the failure modes change.

That change matters. A platform can have a solid kill switch, clean scope validation, and a respectable audit trail when tested as a single-agent system, then behave badly the moment it runs five workers in parallel. One worker halts, another keeps going. Each worker stays inside its local rate budget, but the target still gets flooded in the aggregate. One agent treats a newly discovered asset as merely interesting; another treats the same discovery as implicit authorization to probe it. None of those failures are hypothetical. They are what multi-agent safety looks like in practice.
...
It is not a normative basis for cross-vendor federation or for separately operated platforms that happen to touch the same environment. If separate operators participate in one engagement, the guidance here is still useful, but it should be treated as a coordination baseline rather than proof that shared controls are actually enforced.
...
Multi-agent platforms should preserve a short set of invariants across every active worker and orchestration component.
...
If a platform cannot hold those invariants during concurrent operation, the individual controls may still exist on paper, but the platform is not actually running under the same safety model that APTS assumes.
...
Discovery is not authorization. In a multi-agent design, that point needs to stay painfully clear.
...
State isolation is one of the gaps the issue comments sharpened, and rightly so. Multi-agent systems fail in ugly ways when working context sloshes around without discipline.
